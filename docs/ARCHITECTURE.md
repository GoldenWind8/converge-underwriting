...
the band is a lookup an underwriter can reproduce by hand — and it is mirrored in the
Price page's script so a rating change re-bands live. The same evidence check applies
to findings a reviewer adds on the review page.
