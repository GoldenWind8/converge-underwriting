...
The assessment step (docs/SOLUTION_DESIGN.md §4.1, §6.11).
  ...
  2. memory.retrieve() finds the k most comparable approved cases among those
     with findings in the sections about to be assessed,
  3. one "main" call PER CONFIRMED SECTION proposes findings for that section,
     with the section-filtered precedents (findings plus the reviewer's
     corrections and why-notes) and the raw document in the user message.
...
- precedent_case_ids: cite the precedent cases that informed the finding. Leave empty only if it is genuinely novel.
...
Weigh the precedent cases heavily — they encode this client's human underwriters' past decisions, and the reviewer changes noted on each case are their direct corrections."""
...
def assess_sections(
    ...
    precedents = memory.retrieve(profile, [n.section for n in required], k=5) if use_memory else []
    ...
    if required:
        with ThreadPoolExecutor(max_workers=len(required)) as pool:
            assessments = list(pool.map(
                lambda need: _assess_one_section(raw_text, need, precedents),
                required,
            ))
...
def _render_precedent(case: CaseRecord, section_id: SectionId) -> str:
    """A precedent case's approved findings for one section, followed by the
    reviewer's changes in that section (with their why-notes) — the lesson
    travels with the case. Empty if the case has nothing under this section."""
    relevant = [f for f in case.approved_findings if f.section == section_id]
    section_of = {f.factor_name: f.section for f in case.draft_findings + case.approved_findings}
    changes = [c for c in case.corrections if section_of.get(c.factor_name) == section_id]
    if not relevant and not changes:
        return ""
    findings = "\n".join(
        f"  - {f.factor_name} [{f.severity.value}] {f.reasoning}"
        for f in relevant
    ) or "  (no findings approved in this section)"
    text = (
        f"Case {case.case_id} — {case.summary}\n"
        f"  Outcome: {case.final_band} band\n{findings}"
    )
    if changes:
        text += "\n  Reviewer changes: " + ", ".join(
            f"{c.type} {c.factor_name}"
            + (f" {c.detail.replace(' -> ', '→')}" if c.type == "severity_changed" and c.detail else "")
